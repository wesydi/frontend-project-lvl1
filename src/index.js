...
const evenNumber = (usrName) => {
  ...
  const iter = (number, acc) => {
    const answerUser = readlineSync.question(`Question: ${number}`);
    ...
    if (answerUser === 'yes' && number % 2 === 0 || answerUser === 'no' && number % 2 !== 0) {
      ...
      acc = 0;
      console.log(`Your Answer: ${answerUser}. 'yes' is wrong answer ;(. Correct answer was 'no'. Let's try again, ${usrName}!`);
    }
